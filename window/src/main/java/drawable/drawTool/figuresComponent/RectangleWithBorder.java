...
public class RectangleWithBorder extends Figure {
    ...
    public void draw(Vector2D position, Vector2D size, Painter drawer) {
        ...
        if (secondColor == null) { //without fill
            drawer.drawOnlyBorderRect(tmp.getFirst(), tmp.getSecond(), getMainColor(), borderThickness);
        } else {
            drawer.drawFilledRect(tmp.getFirst(), tmp.getSecond(), getMainColor(), secondColor, borderThickness);
        }
    }
